...
const ProductDetailPage = () => {
  ...
  const fetchProduct = async(id)=>{
    try {
      ...
      const message = error?.response?.data?.message || "Không thể tải sản phẩm";
      const newToast = { id: Date.now(), content: message, typeToast: "error" };
      setToasts((prevToasts) => [...prevToasts, newToast]);
    }
  }
  useEffect(() => {
    ...
    <div>
      ...
      <div className='flex px-[9px]'>
        <div className='w-[58%] pr-[44px]'>
          <div className='mb-5'>
            <img loading="lazy" decoding="async" className='w-full' src={resolveCakeImageSrc(productDetai.image)} alt={productDetai.name || 'product image'} />
          </div>
          <div>
            <img loading="lazy" decoding="async" className='w-full'  src={resolveCakeImageSrc(productDetai.image)}  alt={productDetai.name || 'product image'} />
          </div>
            ...
            <div className=''>
              {/* <div className=''>
                ...
                <div className='flex gap-4'>
                  <button aria-pressed={choicePiece[0]} className={`w-[120px] ease-in duration-500 h-[40px] border border-gray-300 text-xs flex justify-center items-center  ${choicePiece[0]?"bg-black text-white":""} hover:text-white hover:bg-black`} onClick={()=>updatePiece(0)}>6-8 miếng</button>
                  <button aria-pressed={choicePiece[1]} className={`w-[120px] ease-in duration-500 h-[40px] border border-gray-300 text-xs flex justify-center items-center  ${choicePiece[1]? "bg-black text-white":""} hover:text-white hover:bg-black `} onClick={()=>updatePiece(1)}>12-16 miếng</button>
                </div>
              </div> */}
              <div>
